		   ...
		   for (i=0; i<num_unique; i++) {
			...
			// Fast path: no ambiguity codes
    			if (b1 < 4 && b2 < 4 && b3 < 4 && b4 < 4) {
        			...
        			count_array[b1*4 + b2][b3*4 + b4] += site_counter[i];
        			continue;
    			}
			...
			double w12[16], w34[16];
    			for (int a = 0; a < 4; a++)
        		  for (int b = 0; b < 4; b++)
            		    w12[a*4+b] = w1.weight[a] * w2.weight[b];
    			for (int c = 0; c < 4; c++)
        		  for (int d = 0; d < 4; d++)
            		    w34[c*4+d] = w3.weight[c] * w4.weight[d];
    			...
    			for (int row = 0; row < 16; row++) {
        		  ...
        		  for (int col = 0; col < 16; col++)
            		    count_array[row][col] += contrib * w34[col];
    			  }

    			}

    			// printf(" count_sites is %f, ",count_sites);
